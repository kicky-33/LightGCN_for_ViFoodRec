...
def main():
    ...
    median_act = np.median(list(user_activity.values()))
    active_users = {u for u, n in user_activity.items() if n >= median_act}
    less_users = {u for u, n in user_activity.items() if n < median_act}
    ...
    print(
        f"  Active (>= median={median_act:.0f} interactions): "
        f"Recall={r_act:.4f}, NDCG={n_act:.4f} ({len(active_users)} users)"
    )
    print(
        f"  Less-active (< median): "
        f"Recall={r_less:.4f}, NDCG={n_less:.4f} ({len(less_users)} users)"
    )

    summary_lines += [
        "\n=== 2. USER GROUP ANALYSIS ===",
        f"Median interactions per user (train) = {median_act:.0f}",
        f"Active users (>= {median_act:.0f}): n={len(active_users)}, "
        f"Recall@{TOPK}={r_act:.4f}, NDCG@{TOPK}={n_act:.4f}",
        f"Less-active users (< {median_act:.0f}): n={len(less_users)}, "
